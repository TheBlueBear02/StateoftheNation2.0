...
| `src/lib/dreamGovPicksCookie.ts` | Persist user’s cabinet picks in cookie `dream-gov-picks` (office/candidate/party ids; 180 days) |
...
| `src/lib/sharePngImage.ts` | Share helper: Android/Samsung prefer Web Share (skip unreliable image clipboard); iOS/desktop use Safari-safe `ClipboardItem(Promise)`; fallback open-image / download |
...
| `src/components/elections/DreamGovernmentPromo.tsx` | Homepage / elections teaser linking to `/elections/dream-government`; media from `public/dream-government-homepage.png` |
...
Interactive dream cabinet builder. Users pick PM + 6 ministers from confirmed parties' candidate lists, copy a share PNG to the clipboard, and (on successful share) upsert those picks into site-wide stats.

**Link share preview (OG / Twitter):** `src/app/elections/dream-government/page.tsx` sets page `title` / `description` plus `openGraph` and `twitter` with image `public/dream-governemnt-og-image.jpeg` (1280×1600 JPEG; absolute URL via `getSiteUrl()`). This is separate from the in-app clipboard PNG export of the user's cabinet.

1. **Layout** — page header with title `בחרו את ממשלת החלומות שלכם`, subtitle `אם יכולתם לבחור את השרים בבחירות ישירות איך הייתה נראת הממשלה שלכם? לאחר שתבחרו תוכלו לראות כמה אנשים בחרו כמוכם.`, breadcrumb `בחירות 2026 / ממשלת החלומות`. The interactive cabinet lives inside a dark report card (`#040a14`, 12px radius) with a soft topographic contour-line background (`public/dream-government-bg.png` — dense organic dark-on-dark waves pre-composited at ~30% over `#040a14`, shown at full opacity with a light `brightness(1.45) contrast(1.1)` lift so the lines read a bit clearer; on-page and share PNG stay matched): white site logo top-right, share icon top-left, centered card heading (`ממשלת החלומות שלי` only — no `בחירות 2026` subtitle), then a PM portrait (same size as ministers) above a responsive grid of 6 minister portraits. On-page slots use the same face ratio / crop / type hierarchy as the share card (`--dream-face-ratio: 278 / 270`, `object-position: center 18%`, larger party labels, wider `--dream-slot-max: 186px`) so the live board previews the exported image. The offscreen share card uses the same background art.
2. **Offices** — defined in `src/lib/dreamGovernmentOffices.ts`: neutral empty-slot labels `ראש/ת הממשלה`, then `שר/ת הביטחון`, `שר/ת החוץ`, `שר/ת האוצר`, `שר/ת המשפטים`, `שר/ת החינוך`, `השר/ה לביטחון לאומי`. After a pick, `getDreamOfficeLabel` switches from `people.gender`: `זכר` → `שר הביטחון` / `ראש הממשלה` / `השר לביטחון לאומי`, `נקבה` → `שרת הביטחון` / `ראשת הממשלה` / `השרה לביטחון לאומי` (unknown gender keeps the slash form). PM is fully selectable (7 picks total). Titles update on the board and on the share PNG.
3. **Empty / filled portraits** — slots use the share-matched near-square face ratio (`278 / 270`) with rounded 12px frames. Empty slots are dashed with a muted `+`. After a pick, the slot shows the candidate photo (or initials) with no party logo, plus a small pencil edit badge in the **top-right** of the portrait (whole slot stays clickable to reopen the picker). The office title sits **above** the portrait; the person name and party short name (fallback: full name) sit under it. Clicking a filled slot reopens the picker. The same person may be chosen for multiple offices (no dedup). Picks are saved to cookie `dream-gov-picks` (compact `{ officeId, candidateId, partyId }` JSON, `SameSite=Lax`, 180-day max-age) and restored on refresh by reloading those candidates from Supabase once the party list is ready.
4. **Pick stats (site-wide %)** — percentages stay hidden until the user successfully shares (clipboard copy or download fallback). After that share, `dream-gov-has-shared` is set in `localStorage` and a circular `%` badge appears on the **physical bottom-left** of each filled portrait (`DreamOfficeSquare` + `.dream-office__pct-badge`). Value = `round(100 × candidatePicks / officeTotal)` for the selected `election_candidates.id` (sample size stays in the accessible `aria-label` only). Badge uses a dark semi-transparent fill with popularity-colored text and a matching dashed circular border via `getDreamPickPopularity`: **low** `<25%` (gold), **mid** `25–49%` (orange), **high** `50–74%` (brand blue), **top** `≥75%` (purple). One anonymous browser (`client_id`) contributes **one current vote per office** — re-sharing upserts and replaces that browser’s previous picks (so a single tester always sees `100%` until another browser shares). On share, picks are merged into local stats immediately (`applyLocalPicks`) so badges appear even before submit/refetch finishes. If an office still has no aggregate votes, no badge is shown (do not invent `100%`). A single browser that is the only voter for a seat correctly sees `100%` until others share. Stats load via `useDreamCabinetStats` → `GET /api/elections/dream-government/stats` (aggregates from RPC `get_dream_cabinet_pick_stats`). Badges are also rendered on the offscreen share PNG (`ShareableGovernment` + `.dream-share-slot__pct`, same popularity colors). Local picks are applied with `flushSync` before export so the image includes up-to-date percentages. **TEMP mock:** `USE_DREAM_GOV_MOCK_STATS` in `src/lib/dreamGovMockStats.ts` (currently `false`) ignores real aggregates when enabled, unlocks badges immediately, and assigns a random 0–100% per office on each full page reload — keep `false` in production.
5. **Picker modal** — 2 steps: (a) party grid from `useElectionParties` (parties without a list leader disabled, same visual language as the lists-game picker); (b) person grid from `useElectionCandidates(partyId)` — 3 full-bleed portrait tiles per row (image covers the card; list position badge + name overlaid), matching the party-tile look. Escape / backdrop / close button dismiss; back returns to the party step.
6. **Share + persist** — top-left white share icon on the dark card **and** a primary button under the card (`שתפו את ממשלת החלומות שלכם`); both call the same export handler (disabled until at least one office is filled). On successful **clipboard** copy, both controls briefly show **`הועתק ללוח`** instead of their usual content for ~2.5s (icon expands to a pill; bottom button label swaps). Export uses `sharePngImage` (`src/lib/sharePngImage.ts`) + `exportNodeToPng` with `backgroundColor: '#040a14'` so the PNG matches the on-page card (same topographic bg art + `--dream-report-art-opacity`). Mobile Web Share `text` appends the absolute page URL (`/elections/dream-government`). **Mobile Safari / WebKit:** when clipboard is used, `clipboard.write` must start in the same user-gesture turn — the helper passes a `Promise<Blob>` into `ClipboardItem` (export runs inside that promise) instead of awaiting export before `write`. **Android / Samsung Internet:** image clipboard is unreliable and trying it first can burn the user gesture so `navigator.share` never opens — on these browsers the helper **skips clipboard** and prefers the native share sheet; if share is blocked after the async export, it opens the PNG in a new tab for long-press share/save, then `<a download>`. Desktop fallback order: clipboard → Web Share → download. Export clones the offscreen card, routes **every** cross-origin portrait through `/api/image-proxy` (no client-side host allowlist; keeps jpeg/png/webp data URLs as-is; rasterizes the SVG logo to PNG with explicit size), embeds Heebo via `getFontEmbedCSS`, retries `toPng` at `pixelRatio` 2 then 1. After a successful export (copy, share, or download), the page best-effort `POST`s filled seats to `/api/elections/dream-government/submit` with anonymous `clientId` from `localStorage` key `dream-gov-client-id` (`getOrCreateDreamGovClientId`), then refetches stats. Upsert is unique on `(election_id, client_id, office_id)` so re-sharing updates that browser’s votes rather than adding new ones. Submit failure is logged only — it does not block the share flash. Share card is fixed **1080×1350 (Instagram 4∶5)** with wide portrait slots (`--dream-share-slot: 278px`, face ~278×252) so the 3-column grid fills most of the canvas width; office titles are slightly larger than the original export (`27px`) with a little space above the faces. Bottom padding stays ≥40px and the CTA uses `margin-top: auto` so the footer + URL stay pinned and visible when the board is dense. Layout uses `justify-content: flex-start` with tight fixed gaps (not `space-between`) so vertical rhythm stays even. Footer on the image reads `הרכיבו גם את ממשלת החלומות שלכם באתר מצב האומה` with `www.stateofthenation.co.il` on the next line. Notes: same-origin/localhost URLs are **not** upgraded http→https (otherwise the dev logo would be misrouted to the proxy); images already inlined as data URLs are never dropped even if `decode()` times out.

**Schema:** apply [`Layer 1 - Gathering Data/schema_dream_cabinet_picks.sql`](../Layer%201%20-%20Gathering%20Data/schema_dream_cabinet_picks.sql) in Supabase (table `dream_cabinet_picks` + RPC). See [Database.md](./Database.md).

| File | Role |
|------|------|
| `src/lib/dreamGovClientId.ts` | Anonymous UUID in `localStorage` |
| `src/lib/dreamGovPicksCookie.ts` | Cookie restore of in-progress cabinet picks |
| `src/lib/fetchDreamCabinetStats.ts` | Stats fetch + submit helpers + % math |
...
| Stage | Script | Action |
|-------|--------|--------|
| 1 | `resolve_candidates.py` | Match names → `election_candidates` (all pending raw rows) |
...
From `/elections/edit`, stages **2–6 are scoped to the selected `partyId`** (and stage 2 only queries people who still have missing enrichable fields). That keeps Wikidata/OpenAI work bounded to the new list instead of reprocessing every 2026 candidate (which previously hit the 2‑minute stage-2 timeout with `הרצת ה-pipeline נמשכה יותר מדי זמן`). Stage timeouts: 2/3 = 10 min, 4 = 15 min, 5/6 = 5 min. CLI `run_pipeline.py` without `--party-id` still processes all parties.
