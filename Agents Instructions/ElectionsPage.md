...
| `/elections/dream-government` | `src/views/DreamGovernmentPage.tsx` | Dream cabinet builder (PM + 6 offices), clipboard PNG share, site-wide pick % bars |
...
| `src/views/DreamGovernmentPage.tsx` / `.css` | Dream cabinet: pick PM + 6 ministers, share PNG, show site-wide pick % bars |
...
| `src/components/elections/dream/DreamOfficeSquare.tsx` | Empty (+) or filled office/PM square; optional vertical % bar to the physical left of the portrait |
| `src/components/elections/dream/DreamOfficePickerModal.tsx` | 2-step modal: pick party → pick person from `useElectionCandidates` |
| `src/components/elections/dream/ShareableGovernment.tsx` | Fixed-size offscreen cabinet card for clipboard PNG export (no % bars) |
...
4. **Pick stats (site-wide %)** — percentage bars stay hidden until the user successfully shares (clipboard copy or download fallback). After that share, `dream-gov-has-shared` is set in `localStorage` and bars appear for filled seats on the **physical left** of each portrait (`DreamOfficeSquare` + `.dream-office__pct`). Height / label = `round(100 × candidatePicks / officeTotal)` for the selected `election_candidates.id`; a small total under the `%` shows the office sample size (`officeTotal`). One anonymous browser (`client_id`) contributes **one current vote per office** — re-sharing upserts and replaces that browser’s previous picks (so a single tester always sees `100%` / total `1` until another browser shares). Stats load via `useDreamCabinetStats` → `GET /api/elections/dream-government/stats` (aggregates from RPC `get_dream_cabinet_pick_stats`). Bars are **not** included on the offscreen share PNG.
...
| `src/hooks/useDreamCabinetStats.ts` | Load/refetch site aggregates |
