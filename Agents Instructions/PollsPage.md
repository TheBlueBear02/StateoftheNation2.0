...
2. Parser walks in-scope sections: **Seat projections** (+ **Scenario polls** only with `--backfill`). Incremental default keeps **all Seat projections tables under the newest year heading** (Wikipedia splits 2026 across several wikitables); archived older-year subsections and scenarios are skipped. Data rows whose fieldwork cell is a Wikipedia date range (`29–30 Jul`) are treated as poll rows (not header/event rows); `normalize_polls.parse_fieldwork` inherits month/year from the range end when the start token is day-only.
