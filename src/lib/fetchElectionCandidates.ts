...
import { computeMemberTenureStats } from './knessetTenure'
import type {
  ElectionCandidatePerson,
  ElectionCandidateRow,
  KnessetMembershipTenureRow,
} from './supabase'
...
export const EMPTY_CANDIDATE_STATS: ElectionCandidateStats = {
  total: 0,
...
export function buildCandidateMapPins(
  candidates: ElectionCandidate[],
): CandidateMapPin[] {
...
export async function fetchElectionCandidates(
  ...
  partyId: number,
): Promise<{ candidates: ElectionCandidate[]; error: string | null }> {
  ...
  if (queryError) {
    return { candidates: [], error: queryError.message }
  }
  ...
  if (membershipError) {
    return { candidates: [], error: membershipError.message }
  }
  ...
  return {
    candidates: rows.map((row) =>
      normalizeCandidate(row, servedPersonIds, tenureMap),
    ),
    error: null,
  }
}
