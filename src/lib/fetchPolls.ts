...
export function formatFieldwork(start: string, end: string): string {
  const fmt = (d: string) =>
...
export async function fetchPolls(
  ...
  limit = 30,
): Promise<{ polls: PollWithResults[]; error: string | null }> {
  ...
  if (pollError) {
    return { polls: [], error: pollError.message }
  }
  ...
  const { data: publisherRows } = await client
  ...
  const { data: pollsterRows } = await client
    .from('pollsters')
  ...
  const { data: confirmedPartyRows } = await client
